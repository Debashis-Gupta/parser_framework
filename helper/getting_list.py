...
def get_list():
    ...
    print(f"Current data folder path : {data_folder_path}")
    # Access a specific file within the data folder
    path_list = os.path.join(data_folder_path, 'ta1-theia-e3-official-3.json')
    ...
    for i in range(1):
        ...
        if not osp.exists(now_path):
            ...
            break
        f = open(now_path,'r',encoding="utf8")
        fw = open(now_path+'_3.txt','w',encoding="utf8")
        cnt =0
        for line in f:
            cnt +=1
            # if cnt % notice_sum ==0:
        ...
        fw.close()
        f.close()
    return srcId_list, srcType_list, dstID_list, edgeType_list, timestamp_list
